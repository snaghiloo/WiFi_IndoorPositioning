...
waps_only_training[waps_only_training < (0.70)] <- 0

waps_only_validation[waps_only_validation < (0.70)] <- 0
...
B2_svm_PredictResult <- predict(B2_svmModel, newdata = validation_dataSet)
confusionMatrix(B2_svm_PredictResult,validation_dataSet$FLOOR)
# Accuracy   Kappa
#0.9515      0.9342

# train model with ranger to predict floor -----
B2_rangerModel<- train(FLOOR ~., 
                    data = training_dataSet, 
                    method = "ranger",
                    #tuneLength = 3,
                    trControl = ctrl1)
...
confusionMatrix(B2_ranger_PredictResult,validation_dataSet$FLOOR)
# Accuracy   Kappa
#0.944      0.9239

# train model with knn to predict floor ---------------------------
B2_knnModel<- train(FLOOR ~., 
                    data = training_dataSet, 
                    method = "knn",
                    tuneLength = 3,
                    trControl = ctrl1)
B2_knnModel
# k  Accuracy   Kappa    
# 5  0.9724719  0.9642051

B2_knn_PredictResult <- predict(B2_knnModel, newdata = validation_dataSet)
confusionMatrix(B2_knn_PredictResult,validation_dataSet$FLOOR)
# Accuracy   Kappa
# 0.959     0.9444
...
validation_dataSet<-validation_dataSet%>%
  mutate(error.model=abs(as.numeric(B2_knn_PredictResult)-(as.numeric(FLOOR))))  
...
ggplot() + 
  geom_point(data=trainingDF, aes(x= LONGITUDE, y= LATITUDE, color= PHONEID)) +
  xlim(-7425, -7275)+
  ylim(4864725, 4864875)+
  facet_grid(. ~ FLOOR) 
